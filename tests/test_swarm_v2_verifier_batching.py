...
ModelGateway contract. No network, no provider, no paid call.
"""
...
from backend.engines.swarm_v2 import (
    MAX_VERIFIER_BATCH_JSON_BYTES, MAX_VERIFIER_CLAIMS_PER_BATCH, BoundedTaskExecutor,
    EvidenceReference, RemainingBudget, SwarmState, SwarmV2Engine, Verifier,
...
from test_swarm_v2_stage1_e2e import Plans, Worker, commander
...
def test_all_deterministic_claims_consume_zero_verifier_model_calls():
    ...
    verdicts = Verifier(gateway=gateway, model="fake").verify(
...
def test_one_small_batch_makes_exactly_one_call_with_the_exact_payload():
    items = refs(3)
    gateway = RecordingGateway()
    verdicts = Verifier(gateway=gateway, model="fake").verify(items)
    ...
    assert gateway.payloads[0] == serialize_verifier_candidates(items)
...
def test_claim_count_split_is_deterministic_and_respects_both_bounds():
    ...
    gateway = RecordingGateway()
    Verifier(gateway=gateway, model="fake").verify(items)
    ...
    for payload in gateway.payloads:
        ...
        assert len(decoded) <= MAX_VERIFIER_CLAIMS_PER_BATCH
...
def test_byte_size_split_happens_below_the_claim_count_limit():
    ...
    assert verifier_payload_bytes(items) > MAX_VERIFIER_BATCH_JSON_BYTES
    gateway = RecordingGateway()
    Verifier(gateway=gateway, model="fake").verify(items)
...
def test_single_oversized_candidate_fails_closed_before_any_provider_call():
    ...
    assert verifier_payload_bytes([items[1]]) > MAX_VERIFIER_BATCH_JSON_BYTES
    gateway = RecordingGateway()
...
def test_three_batches_are_three_real_verifier_model_calls():
    gateway = RecordingGateway()
    verdicts = Verifier(gateway=gateway, model="fake").verify(refs(60))
...
def test_shuffled_input_produces_identical_batches_and_verdict_order():
    ...
    ordered = Verifier(gateway=first, model="fake").verify(items)
    reordered = Verifier(gateway=second, model="fake").verify(shuffled)
...
def test_reordered_response_is_accepted_and_final_order_stays_deterministic():
    def responder(claim_ids, _index):
    ...
    verdicts = Verifier(gateway=gateway, model="fake").verify(refs(4))
...
def test_omission_is_local_to_its_own_batch():
    def responder(claim_ids, index):
        answered = claim_ids[1:] if index == 2 else claim_ids
        return {"verdicts": [{"claim_id": claim_id, "verdict": "verified", "reason": "ok"}
                             for claim_id in answered]}
    gateway = RecordingGateway(responder)
    verdicts = Verifier(gateway=gateway, model="fake").verify(refs(30))
...
def test_duplicate_response_claim_id_fails_closed_without_last_wins():
    ...
    with pytest.raises(VerifierContractError) as excinfo:
        Verifier(gateway=RecordingGateway(responder), model="fake").verify(refs(2))
...
def test_verifier_batches_run_strictly_sequentially():
    ...
    Verifier(gateway=gateway, model="fake").verify(
...
def test_empty_and_complete_checkpoints_both_remain_valid():
    items = refs(3)
    assert len(plan_verification(items, existing_verdicts={}).batches) == 1
    complete = {item.claim_id: {"claim_id": item.claim_id, "verdict": "verified",
                                "reason": "ok"} for item in items}
    resumed = plan_verification(items, existing_verdicts=complete)
...
def test_malformed_checkpoint_verdict_fails_closed():
    with pytest.raises(VerifierContractError) as excinfo:
        plan_verification(refs(2), existing_verdicts={
...
def test_checkpoint_contradicting_a_deterministic_verdict_fails_closed(
    ...
    with pytest.raises(VerifierContractError) as excinfo:
        plan_verification(items, conflict_claim_ids=conflicts,
...
def test_resume_skips_checkpointed_claims_and_matches_an_uninterrupted_run():
    ...
    expected = Verifier(gateway=uninterrupted_gateway, model="fake").verify(items)
    assert uninterrupted_gateway.model_calls == 3

    first_batch = build_verifier_batches(items)[0]
    ...
    resumed = Verifier(gateway=resumed_gateway, model="fake").verify(
        items, existing_verdicts=checkpoint)
...
def test_an_uncheckpointed_batch_is_replayed_rather_than_falsely_skipped():
    ...
    expected_batches = [[item.claim_id for item in batch]
                        for batch in build_verifier_batches(items)]
    ...
    with pytest.raises(RuntimeError):
        Verifier(gateway=crashed, model="fake").verify(
            items, batch_completed=crash_after_the_second_batch)
    ...
    resumed_gateway = RecordingGateway()
    resumed = Verifier(gateway=resumed_gateway, model="fake").verify(
        items, existing_verdicts=durable)
...
def engine_with(gateway, evidence_refs, *, remaining=None, checkpoints=None,
    ...
    engine = SwarmV2Engine(
        ...
        verifier=Verifier(gateway=gateway, model="fake"),
