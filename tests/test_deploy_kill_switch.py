...
def run_switch(tmp_path, *args, env=None, api=CLOSED_API, worker=CLOSED_WORKER, capture=None,
               ack=True, capture_configured=True):
    ...
    (mock_dir / "job.json").write_text(json.dumps(worker))
    ...
    result = subprocess.run(["bash", str(KILL_SWITCH), "--operator-config", str(config), *args],
                            capture_output=True, text=True, env=full_env, timeout=120)
    return result, [line for line in log.read_text().splitlines() if line]


def mutations(calls):
    return [c for c in calls if not re.match(r"gcloud (auth list|config get-value|run (jobs|services) describe)", c)]
