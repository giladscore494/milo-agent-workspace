...
| 4 | `04-create-probes.sh` | creates 2 disposable jobs | `stagec-db-probe` (as `milo-api-runtime@`; DB checks/setup/evidence) and `stagec-gw-probe` (as `milo-vercel-gateway@`; drives the API); probe sources ship as deterministic gzip+base64 (`PROBE_SOURCE_GZIP_B64`, reconstructed to exact UTF-8 bytes in-container) with a pre-mutation check that no env value exceeds Cloud Run's 32,768-character limit |
...
| 8 | `08-recover-evidence.md` (**one manual trap-guarded block**) | API run-creation flag (temporarily), probes (stale ones reconciled, recreated, deleted) | Attempt 7 evidence recovery for run `8b4a4277-…` ONLY: after the corrective is merged, reconcile any stale probes (either transport generation, verified by exact name/posture, deleted — or already absent since recovery attempt 1's cleanup) and recreate both from corrected source, replay + rerun the evidence gate with the worker paid flag `false`, all provider aliases absent and the launcher `disabled`, prove no new run/execution; cleanup traps (EXIT/ERR/INT/TERM, armed before any mutation) restore `MILO_ENABLE_RUN_CREATION=false`, run `kill-switch.sh` and delete both probes on every exit path. Authorizes NO new paid run |
