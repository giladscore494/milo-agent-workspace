#!/usr/bin/env bash
# Stage D step 4: create the two DISPOSABLE probe jobs (deleted in step 7
# / the lockdown cleanup).
...
# bytes inside python:3.12-slim before exec. Raw transport is never used:
# Cloud Run caps a single env value at 32,768 characters and probe_db.py
# raw exceeds it. Neither job image is custom.
set -euo pipefail
...
echo "== Creating ${STAGE_D_DB_PROBE_JOB} (SA: ${STAGE_D_API_SA})"
...
echo "== Creating ${STAGE_D_GW_PROBE_JOB} (SA: ${STAGE_D_GATEWAY_SA}, no secrets)"
...
echo "OK: probe jobs created (disposable; compressed transport; deleted by 07-post-run-lockdown.sh)."
