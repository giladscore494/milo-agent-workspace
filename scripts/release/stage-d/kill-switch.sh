#!/usr/bin/env bash
# Stage D emergency kill switch — run at ANY sign of trouble (invariant
# failure, unexpected cost, anomalous behavior). Applies ROLLBACK.md
# emergency order: paid off + catalog off → run creation off → launcher off
# → provider key bindings removed (EVERY accepted alias) → cancel any
# genuinely active worker execution → unconditional final zero-active
# verification → verify the fail-closed postconditions.
#
...
#
# It deliberately does NOT delete the probe jobs: probe deletion belongs to
# 07-post-run-lockdown.sh, which verifies their absence. A kill switch that
# silently removed evidence-collection capability would be the wrong tool
# in an incident.
set -euo pipefail
...
# the durable catalog — and the prepared Government capture run — are exactly
# as they were when the flag is set back.
if ! gcloud run jobs update "${STAGE_D_WORKER_JOB}" \
...
if ! gcloud run services update "${STAGE_D_API_SERVICE}" \
    --project="${STAGE_D_PROJECT}" --region="${STAGE_D_REGION}" \
...
cancel_listed() {
  ...
  while IFS= read -r execution; do
    ...
    if ! gcloud run jobs executions cancel "${execution}" \
        --project="${STAGE_D_PROJECT}" --region="${STAGE_D_REGION}" --quiet; then
...
verify_api_posture() {
  ...
  if ! gcloud run services describe "${STAGE_D_API_SERVICE}" \
      --project="${STAGE_D_PROJECT}" --region="${STAGE_D_REGION}" \
...
if ! verify_worker_posture; then
  note_failure "worker postcondition failed (paid flag / catalog flag / provider alias bindings)"
fi
...
echo "KILL SWITCH APPLIED: paid off, catalog off, all provider key aliases unbound, run creation off, launcher disabled, no active worker executions."
