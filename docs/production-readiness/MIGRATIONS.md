...
| ts | `20260925000100_reasoning_aware_usage.sql` | PR-R reasoning-aware usage accounting (MILO_V2_REASONING_BUDGET_PR_SPEC.md 4.5). `run_usage_ledger` gains six NULLABLE per-call columns — `cached_input_tokens`, `cache_write_tokens`, `reasoning_tokens` (provider-reported), `reasoning_tokens_estimated` + `reasoning_estimated` (completion − answer, only when reasoning was not reported), `answer_tokens` — where NULL means "not reported", never zero, plus a non-negative CHECK. `append_usage_ledger_guarded` is restated with the same signature, lease fence and service-only ACL to write them. `execution_usage_public_projection` is widened by the run-level sums of the same breakdown (the closed `RunUsage` contract, widened as one decision). Counts only: no reasoning text is ever stored. Additive and forward-only. Mirrors `backend/model_usage.py` / `backend/budget.py` |
...
`UPDATE` / `DELETE` (the append-only trigger refuses both to every role). Every
new function is `EXECUTE` for `service_role` alone.
