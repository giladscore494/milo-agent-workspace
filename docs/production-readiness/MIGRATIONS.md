...
| ts | `20260921000200_immutable_run_identity.sql` | Immutable run identity and the last unfenced worker writes. `runs.run_identity` (nullable, so pre-existing runs stay valid) records what a run IS — workflow key, reviewed engine version, runtime policy version/fingerprint, release SHA and event-registry version — bound ONCE before execution by `bind_run_identity` (writes only while the column is null, idempotent for an identical re-bind, refuses a different record) and never rewritten afterwards: `runs_forbid_identity_rewrite` refuses ANY update that changes a non-null identity, through an RPC, a direct service-role write or an erasure, and a shape constraint rejects a record that is not one. Plus `create_tool_access_request_guarded`, `create_tool_grant_guarded` (both tables in one body, the request must belong to THIS run) and `append_usage_ledger_guarded` (the run id comes from the FENCED argument, never the payload) — the three worker writes that had no lease fence at all, now on the SAME `assert_worker_lease` as every other guarded write. Additive, rerun-safe, service-path only |
