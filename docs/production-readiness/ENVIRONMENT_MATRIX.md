...
| `MILO_ENABLE_RUN_CREATION` | api | no | no | must be off until Stage C | must be off | operator (staged) | `check_unsafe_defaults.py`; smoke tests | off | set off (step 3 of the [ROLLBACK.md](ROLLBACK.md) emergency order) |
...
| `MILO_ENABLE_WORK_SCOPE_MUTATIONS` | api | no | no | off until the Mapping Plan is being used | must be off | operator (staged) | `check_unsafe_defaults.py`; `check-production-config.sh`; Stage A deployment contract; `ExecutionSurfaceGuardMiddleware` | off | set off — plans are drafts; nothing executes from one, the plan reads keep answering, and no row is deleted or changed |
| `MILO_ENABLE_WORK_SCOPE_BATCHES` | api | no | no | off until a prepared Mapping Plan batch is to be started from the website (with `MILO_ENABLE_RUN_CREATION`) | must be off | operator (staged) | `check_unsafe_defaults.py`; `check-production-config.sh`; Stage A deployment contract; `ExecutionSurfaceGuardMiddleware` | off | set off — no batch starts, pauses or resumes; a running batch is untouched and still cancellable; the progress read keeps answering and no row is deleted or changed |
| `MILO_ENABLE_WORK_SCOPE_PREPARATION` | worker (capture job only) | no | no | off; turned on for ONE capture-job execution by an explicit operator command | must be off | operator (per execution; after the vocabulary evidence) | `check_unsafe_defaults.py`; `check-production-config.sh`; Stage A deployment contract; capture-job definition pins it false | off | set off — the capture entrypoint refuses the scoped mode before any transport or repository exists; no row is deleted or changed |
| `MILO_ENABLE_PAID_EXECUTION` | api+worker | no | no | must be off until Stage C | must be off | operator (Stage C) | fail-closed without budgets+key | off | set off (step 2 of the [ROLLBACK.md](ROLLBACK.md) emergency order) |
| `MILO_ENABLE_CATALOG_EXECUTION` | worker | no | no | must be off until separately authorized | must be off | operator (separate authorization; never a release side effect) | `check_unsafe_defaults.py`; `check-production-config.sh`; Stage A deployment contract; `verify_caps.py` | off | set off — the independent catalog kill switch; no code rollback needed; deletes/mutates no catalog row |
