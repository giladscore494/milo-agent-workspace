...
| Provider transport failures (`run.error.code` / `run_failed` in `PROVIDER_REQUEST_DEADLINE_EXCEEDED`, `PROVIDER_STREAM_INACTIVITY_TIMEOUT`, `PROVIDER_STREAM_INTERRUPTED`, `PROVIDER_CONNECTION_FAILED`, `PROVIDER_HTTP_<status>`) | run rows/events; `jsonPayload.event="provider_call"` in Cloud Logging | MEDIUM→HIGH if repeated | read the run's `provider_call` lines (timings, `finish_reason`, usage, exception class); a quarantined lease follows every deadline/inactivity/interrupted outcome — see REASONING_SAFE_TRANSPORT.md |
| Ownership probe failures (`provider_lease_ownership_lost` / `"event":"provider_lease_probe"`) | run events; Cloud Logging | LOW (observability only) | `exception_class` / `failure_kind` say why; see REASONING_SAFE_TRANSPORT.md |
