...
```bash
# 3. Read-only audit. PREFERRED: zero-secret audit against the stored
#    non-secret metadata generated by the previous apply.
...
1. **Zero-secret audit (preferred)** — `--audit-metadata <file>` points at the
   non-secret `milo-production.metadata.env` generated by a previous apply.
   The file is validated fail-closed (db id present, 16-hex token fingerprint,
   positive numeric pinned version, canonical `*.upstash.io` REST URL —
   missing/malformed metadata is BLOCKED, never worked around), then the LIVE
   Secret Manager state must match it exactly: newest enabled version equals
   the stored pin and its in-memory fingerprint equals the stored fingerprint.
   No Upstash management credential is used at all.
...
to rerun. With `--audit-metadata`, `--audit-only` needs no management key at
all: it verifies consistency via the stored pinned version and the in-memory
fingerprint, returning BLOCKED when exact consistency cannot be proven.
...
service-account keys). Both jobs install the Vercel CLI at the exact version in
