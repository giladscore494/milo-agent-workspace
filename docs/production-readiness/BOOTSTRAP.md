...
#    management credential. Must run against the applied bootstrap SHA.
...
- Generates the manifest plus `bootstrap-plan.json` / `bootstrap-apply.json`
  in a **private operator directory outside the worktree**; the bootstrap-bound
...
1. **Metadata-assisted credentialed audit (preferred)** — `--audit-metadata
   ...
   - `MILO_BOOTSTRAP_SHA` must be a full 40-char SHA **and equal the audited
     `--release-sha`** (metadata from a different bootstrap reconciliation is BLOCKED);
   ...
   `MILO_REDIS_SECRET_VERSION`, `MILO_BOOTSTRAP_SHA`, `UPSTASH_REDIS_REST_URL`)
   equals the stored values on **both** the API service and the worker job —
   any differing Redis field, or a differing bootstrap SHA, is BLOCKED.
...
- it is bootstrap-bound (`MILO_BOOTSTRAP_SHA`), project- and environment-bound
...
- the apply also writes the same identity metadata into the live Cloud Run
  env (`MILO_BOOTSTRAP_SHA`, `MILO_REDIS_DB_ID`, `MILO_REDIS_TOKEN_FINGERPRINT`,
