...
| B11 | supervisor shadow (`worker/main.py:283-291`), `backend/supervisor.py` (422) | — | **ACTIVE (shadow) → D2**; controlled-autonomy block `supervisor.py:211-430` **TEST_ONLY** | worker imports only the shadow names (`worker/main.py:17`) | `hasattr(repo, "create_supervisor_decision")` (`:283-284`) | `tests/test_controlled_supervisor_autonomy.py` (controlled block) | — | — | Line numbers moved from the pre-scan (272-296 → 283-291). `initial_blackboard` → `build_milo_blueprint()` (`supervisor.py:128`), so V2 depends on V1's `workflow.py` |
...
| S9 | `check-production-config.sh` inventory | — | **gap (add, not delete)** | — | — | — | **Still open at HEAD:** 0 occurrences of `MILO_RELEASE_SHA` or `GATEWAY_ALLOW_RUN_START_ROUTES`. PR #124 added only the model-contract block (`:346-368`). `MILO_RELEASE_SHA` is also absent from `ENVIRONMENT_MATRIX.md` |
...
| **1d** | `frontend/components/run/RunOutputPanel.tsx` (F1) | `frontend/scripts/static-ui-check.mjs:44` `'Final artifacts'` → `'Pipeline quality'`, and rewrite `:45-49` to name the V1 typed result panel; `docs/swarm-v2-final-result.md:5,22` | `staticUiCheck.test.ts:25` (marker list) and `:35-40` (`rmSync` target → `components/result/VehicleCatalogResultPanel.tsx`, expected stderr → `Missing UI marker: Pipeline quality`); `finalResultPanel.test.tsx:14` import, delete tests at `:269-280` (5c/5d) and `:528-535` (9d) | — |
...
| **1h** | `Settings.rate_limit_per_minute` and `Settings.environment` (`backend/config.py:13,19`) (B14) | `check-production-config.sh:80` (inventory row `RATE_LIMIT_PER_MINUTE`); `ENVIRONMENT_MATRIX.md:32`. `ENVIRONMENT` itself stays (read directly at `production_config.py:118,390`, `gateway_auth.py:86`) | none | — |
...
|---|---|---|---|
| **2a** | `scripts/release/swarm-v2-smoke/` (6 files, 982 lines) (S2) | remove `scripts/release/swarm-v2-smoke/*.sh` from the shellcheck command in `ci.yml:59` (not locked); rewrite the `parse_env_contract` rows in `RUNTIME_POLICY.md:18`, `ENVIRONMENT_MATRIX.md:44`, `STAGED_ACTIVATION.md:80`; HISTORICAL banner on `docs/deployment/swarm-v2-smoke.md` (stays, locked); `FINAL_ACCEPTANCE.md:68` | delete `tests/test_release_tooling_swarm_smoke.py`; remove `test_the_smoke_env_contract_keeps_the_catalog_flag_off_in_every_posture` (`test_catalog_execution_flag.py:496-502`). Its property (catalog flag off in every posture) is still covered for the **current** contract by `deployment-contract.sh` + `check_unsafe_defaults.py`. Verify before deletion that a current-contract test asserts `MILO_ENABLE_CATALOG_EXECUTION=false` at rest; if none does, port the assertion |
...
| **3b** | remove the mock re-export from `backend/tools/__init__.py:4-6` (B7); **keep** `tools/mock.py` | — | 4 imports → `from backend.tools.mock import …` (`test_swarm_v2_tool_contract.py:45`, `test_swarm_v2_runtime.py:31`, `test_swarm_v2_worker_output_repair.py:38`, `test_swarm_v2_r3_evidence_contract.py:53`) |
| **3c** | remove the reconcile re-export from `government/__init__.py:38-39,44-52` (B6); the module stays until D9 | — | none (tests import the submodule) |
...
| **3e** | S9: add `MILO_RELEASE_SHA` and `GATEWAY_ALLOW_RUN_START_ROUTES` to the `check-production-config.sh` inventory (and `MILO_RELEASE_SHA` to `ENVIRONMENT_MATRIX.md`) | — | — |
| **3f** | delete `gateway_auth_configured` (`gateway_auth.py:80`), `RunCheckpoint` (`schemas.py:328`), `profile_summary` (`model_profiles.py:262` + `__all__` `:283`) | — | none. Verify `event_registry.fingerprint()` is unchanged (it hashes event schemas, and `RunCheckpoint` is not registered) |
...
Not in PR-3 (evidence shows they are locked): B4 (D10), B5, and every B8 method
except `list_unread_agent_messages`.
...
| D2 | supervisor shadow | (a) keep; (b) remove shadow (`worker/main.py:283-291`) and `supervisor.py` | frees V2 from `vehicle_catalog_v1/workflow.py`; the 3 tables stay (8/13/8 rows) | writes stop for `run_blackboards`/`agent_messages`/`supervisor_decisions`; event `supervisor_shadow_failed` leaves the registry (**event fingerprint changes**) |
...
| D5 | kill switch | (a) new `scripts/deploy/kill-switch.sh` covering every current flag incl. Vercel; (b) rewrite `ROLLBACK.md` as manual-only | then stage-c can go (2c) | today the only scripted kill switch misses the whole current production path (§2.3 S3) |
...
3. **S9: check-production-config inventory gap.** It is missing `MILO_RELEASE_SHA` and `GATEWAY_ALLOW_RUN_START_ROUTES` (still true after PR #124), and `ENVIRONMENT_MATRIX.md` lacks `MILO_RELEASE_SHA`. Fix in set 3e.
4. **The only scripted kill switch misses the current path** (S3 / D5). It covers none of `MILO_ENABLE_WORK_SCOPE_*`, `MILO_ENABLE_GOVERNMENT_CATALOG_READ`, `MILO_ENABLE_CATALOG_PROMOTION` or any Vercel gateway variable, and `stage-c/*.sh` is not shellchecked in CI.
...
8. **Stale statements.**
   ...
   - Stage D docs and scripts (`stage-d-env.sh:70`, `verify_images.py:11`, `01-verify-release-images.sh:12`, `STAGE_D_AUTHORIZATION.md:222`) describe an unpinned `openai>=1.30.0`, while `backend/requirements.txt` pins `openai==3.16.2`.
...
Environment: Python 3.11 venv with `backend/requirements.txt`, the CI env of
