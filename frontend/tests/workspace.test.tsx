...
it('does not expose active run or proposal controls',()=>{ render(<Page/>); expect(screen.getByRole('button',{name:/Create workflow proposal \(disabled\)/})).toBeDisabled(); expect(screen.getByRole('button',{name:/Retry disabled/})).toBeDisabled(); expect(screen.getByRole('button',{name:/Cancel disabled/})).toBeDisabled(); });
