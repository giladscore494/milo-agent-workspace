...
export function useRunRealtime(runId?: string) {
  ...
  useEffect(() => { if (!runId) return; let closed = false; api.run(runId).then(run => api.events(runId).then(events => { if (!closed) events.forEach(e => dispatch(e)); return {run, events}; }).then(({run,events}) => reconstructRun(run,events))).catch(()=>{});
    let sb: any; let channel: any; supabaseBrowser().then(client => { sb = client; channel = sb?.channel(`run:${runId}`).on('postgres_changes', { event:'INSERT', schema:'public', table:'run_events', filter:`run_id=eq.${runId}` }, payload => dispatch(payload.new as any)).subscribe((status: string) => setMode(status === 'SUBSCRIBED' ? 'realtime' : 'polling')); });
