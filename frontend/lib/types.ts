...
export type WorkspaceState = { run?: Run; events: RunEvent[]; lastEventId?: number; agents: Record<string, AgentState>; sources: SourceRecord[]; claims: any[]; conflicts: ConflictRecord[]; currentPhase: string; progress: number; tokens: number; cost: number; supervisor: string[]; validationErrors: any[]; checkpoints: any[]; rawErrors: any[] };
