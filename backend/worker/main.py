...
def execute_run(run_id: UUID, repo: Repository, engine: Engine | None = None, budget_tracker: "BudgetTracker | None" = None) -> int:
    ...
    try:
        ...
        tracker = budget_tracker or BudgetTracker(
            ...
            daily_user_reserver=(lambda amount: repo.reserve_daily_user_budget(run_id, run.get("requested_by"), amount, budget_config.daily_user_budget)) if hasattr(repo, "reserve_daily_user_budget") and run.get("requested_by") and budget_config.daily_user_budget else None,
            daily_project_reserver=(lambda amount: repo.reserve_daily_project_budget(run_id, str(ledger_project_id), amount, budget_config.daily_project_budget)) if hasattr(repo, "reserve_daily_project_budget") and ledger_project_id and budget_config.daily_project_budget else None,
        )
