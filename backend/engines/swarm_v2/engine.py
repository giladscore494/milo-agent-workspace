...
class SwarmV2Engine:
    ...
    def _save(self, state: SwarmState) -> None:
        if self._checkpoint_sink:
            self._checkpoint_sink("swarm_v2", {"phase": "swarm_v2", "version": state.engine_version,
    ...
    def run(self, run: dict[str, Any]) -> dict[str, Any]:
        ...
        while True:
            def persist(result: TaskResult) -> None:
                ...
                state.task_outputs[result.task_id] = dict(result.output or {})
            ...
            conflicts: dict[tuple[str, str], list[EvidenceReference]] = {}
            for item in evidence:
                conflicts.setdefault((item.field, repr(item.value)), []).append(item)
            values_by_field: dict[str, set[str]] = {}
            for item in evidence: values_by_field.setdefault(item.field, set()).add(repr(item.value))
            conflict_ids = {item.claim_id for item in evidence if len(values_by_field[item.field]) > 1}
            ...
            summary = {"completed": sorted(completed),
                ...
                "conflicts": sorted(conflict_ids), "remaining_budget": dict(self._usage_snapshot())}
            ...
            self._save(state)
            return self._builder.build(evidence, verdicts)
